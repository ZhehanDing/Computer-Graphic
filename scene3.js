...
for (let i = 0; i < nv; i++) {
   let row = [];
   for (let j = 0; j < nu; j++) {
      let A = [vertices[i + 1][j][0], vertices[i + 1][j][1], vertices[i + 1][j][2]];
      let B = [vertices[i + 1][j + 1][0], vertices[i + 1][j + 1][1], vertices[i + 1][j + 1][2]];
      let C = [vertices[i][j + 1][0], vertices[i][j + 1][1], vertices[i][j + 1][2]];
      let D = [vertices[i][j][0], vertices[i][j][1], vertices[i][j][2]];
...
*/
   ...
   for (let i = 0; i <= nv; i++) {
      for (let j = 0; j <= nu; j++) {
         let u0 = j - 1 < 0 ? nu - 1 : j - 1;
         let v0 = i - 1 < 0 ? 0 : i - 1;

         let u1 = j == nu ? 0 : j;
         let v1 = i == nv ? nv - 1 : i;

         let f0 = facenormal[v0][u0]; 
         let f1 = facenormal[v1][u0];
         let f2 = facenormal[v0][u1];
         let f3 = facenormal[v1][u1];
...
*/
   ...
   for (let i = 0; i < nv; i++) {
      for (let j = 0; j < nu; j++) {
         ...
         mesh.push(vertices[i + 1][j]);

         mesh.push(vertices[i + 1][j]);
         mesh.push(vertices[i + 1][j + 1]);
         mesh.push(vertices[i][j + 1]);
      }

      mesh.push(vertices[i][nu]);
      mesh.push(vertices[i + 1][0]);
   }
...
*/
  ...
  S.BSplineFunction = (keys, n, t) => {
   ...
   S.createSplineSurfaceMesh = (nu, nv, keys) => S.uvMesh((u, v, keys) => {
      let zrInner = S.evalSpline(keys[0], v, S.CatmullRomFunction);
      let zrOuter = S.evalSpline(keys[1], v, S.CatmullRomFunction);
      let zrBend  = S.evalSpline(keys[2], v, S.CatmullRomFunction);
      return [(zrOuter.r - zrInner.r) * u + zrInner.r, v, zrBend.r,
            0, 0, 0, 
            u, v];
   }, nu, nv, keys);
...
*/
      ...
      let X = []; //A VECTOR NOT ALIGNED WITH Z
      let Y = [];
...
render: `
   ...
   S.spinnerMesh = S.createRevolutionMesh(16, 32, S.spinner);
   ...
   m.save();
      ...
      m.roty(time);
      m.rotx( Math.PI/2);
         S.drawMesh(S.spinnerMesh, m.get(), 1);
      m.restore();
   m.restore();
`,
events: `
